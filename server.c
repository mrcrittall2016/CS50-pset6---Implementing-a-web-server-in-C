...
char* indexes(const char* path)
{
    // if path is readable/executable
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (strcasecmp(name, "index.php") == 0 || strcasecmp(name, "index.html") == 0)
        {
            
            // malloc memory for path
            char* direct = malloc(strlen(path) + strlen(name) + 1);
            ...
            // combine directory path with file 
            strcpy(direct, path); 
            strcat(direct, name);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0; 
    ...
    // buffer for bytes (512)
    BYTE buffer[BYTES];
    
    while (true)
    {
        // read buffer's worth of bytes
        int bytes = fread(buffer, sizeof(BYTE), BYTES, file); 
...
const char* lookup(const char* path)
{
    // find full stop in path where file extension should be 
    ...
    else if (0 == strcasecmp(extension, "html"))
    {  
        extension = "text/html";
        return extension; 
    }
    
    else if (0 == strcasecmp(extension, "gif"))
    {  
        extension = "image/gif";
        return extension; 
    }
    
    else if (0 == strcasecmp(extension, "ico"))
    {  
    ...
    else if (0 == strcasecmp(extension, "php"))
    {  
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char method[temp - line + 1]; 
    strncpy(method, line, temp - line); 
    method[temp - line] = '\0';    
    
    // removes first space in temp string
    temp++; 
    
    // find second space and save protocol 
    protocol = strchr(temp, ' ');
    protocol++; 
    
    // create memory for and store request_target
    char request_target[protocol - temp + 1];
    strncpy(request_target, temp, protocol - temp);
    request_target[protocol - temp] = '\0'; 
    ...
    if (0 != strcmp(protocol, "HTTP/1.1\r\n"))
    {    
        error(505);
        return false;
    }
  
    temp = strstr(request_target, "name="); 
    int length = strlen(request_target);
 
    // several switches to take care of optional '?'
    if (temp != NULL)
    { 
        // copy string pointed to by temp to query memory address
        strcpy(query, temp); 
        
        // if find '?' in request_target
        if (strchr(request_target, '?') != NULL)
        {   
            strncpy(abs_path, request_target, temp - request_target - 1);
            abs_path[temp - request_target - 1] = '\0';       
        }
        else 
        {
            strncpy(abs_path, request_target, temp - request_target);
            abs_path[temp - request_target] = '\0';
        }         
    }
    ...
    else 
    {
        ...
        {
            strncpy(abs_path, request_target, length - 2);
            abs_path[length - 2] = '\0';  
        }
